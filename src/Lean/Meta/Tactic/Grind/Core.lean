...
private def closeGoalWithTrueEqFalse : GoalM Unit := do
  ...
  unless (← mvarId.isAssigned) do
    ...
    let falseProof := mkApp4 (mkConst ``Eq.mp [levelZero, levelZero]) (← getTrueExpr) (← getFalseExpr) trueEqFalse (mkConst ``True.intro)
...
private def closeGoalWithValuesEq (lhs rhs : Expr) : GoalM Unit := do
  ...
  let falseProof := mkApp4 (mkConst ``Eq.mp [levelZero, levelZero]) p (← getFalseExpr) pEqFalse hp
  closeGoal falseProof
